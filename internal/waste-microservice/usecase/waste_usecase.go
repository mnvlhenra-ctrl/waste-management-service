...
func (u *WasteUsecase) Create(
	...
	if waste.StockAvailability < 0 ||
		waste.SeparatedCosts < 0 ||
		waste.NonSeparatedCosts < 0 {
		return errors.New(
			"stock availability and costs cannot be negative",
		)
	}

	if !isValidCategory(waste.Category) {
		return errors.New(
			"invalid category. allowed: Organik, Anorganik, B3, Residu",
		)
	}

	waste.Category = strings.Title(
		strings.ToLower(waste.Category),
	)
